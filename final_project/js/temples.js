...
const createCard=function(temple){
    ...
    history.innerHTML=`<p>Important dates:</p><ul>${historyLi.join('')}</ul>`;
    services.innerText=temple.services;
    let closuresLi=temple.closure.map((line)=>`<li>${line}</li>`);
    closure.innerHTML=`<p>Closures:</p><ul>${closuresLi.join('')}</ul>`;
    btnOne.innerText=1;
    btnTwo.innerText=2;
    ...
    secondPart.appendChild(history);
    secondPart.appendChild(closure);
    card.appendChild(firstPart);
    card.appendChild(secondPart);
    card.appendChild(btnOne);
    card.appendChild(btnTwo);
